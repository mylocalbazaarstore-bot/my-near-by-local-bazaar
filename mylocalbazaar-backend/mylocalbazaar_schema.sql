...
-- Phase 1 — Goal 1.1: Database Schema Design
-- Version: 1.0.0
-- ============================================================
...
CREATE EXTENSION IF NOT EXISTS "postgis";         -- for geolocation/radius search
...
CREATE TYPE order_status AS ENUM (
    'payment_pending',
    'payment_processed',       -- Payment done, awaiting merchant approval
    'merchant_approved',       -- Merchant accepted
    'merchant_rejected',       -- Merchant rejected
    'admin_overridden',        -- Admin overrode merchant decision
    'accepted',
    'packed',
    'out_for_delivery',
    'delivered',
...
-- (Pincode-based Hyperlocal Discovery)
-- ============================================================
...
CREATE TABLE areas (
    ...
    name            VARCHAR(150) NOT NULL,               -- e.g. "Kharghar Sector 12"
    pincode         VARCHAR(10) NOT NULL,
    latitude        DECIMAL(10, 8),
    longitude       DECIMAL(11, 8),
    geom            GEOGRAPHY(POINT, 4326),              -- PostGIS point for radius queries
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE users (
    ...
    password_hash       TEXT,                            -- NULL for OTP-only login
    gender              gender,
    date_of_birth       DATE,
...
CREATE TABLE otp_logs (
    ...
    purpose         VARCHAR(50) NOT NULL,               -- login | register | reset | 2fa | delivery
...
CREATE TABLE user_sessions (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id         UUID NOT NULL,                      -- references users, merchants, admins, delivery
    user_role       user_role NOT NULL,
    token_hash      TEXT NOT NULL UNIQUE,               -- hashed JWT
    device_info     JSONB,                              -- {os, browser, ip, device_id}
    ip_address      INET,
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
    expires_at      TIMESTAMPTZ NOT NULL,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TABLE merchants (
    ...
    store_slug              VARCHAR(300) UNIQUE NOT NULL,   -- URL-safe store identifier
    ...
    delivery_radius_km      DECIMAL(5, 2) DEFAULT 5.00,    -- Merchant-controlled delivery zone
    min_order_value         DECIMAL(10, 2) DEFAULT 0.00,   -- Merchant-set minimum order
...
CREATE TABLE merchant_kyc (
    ...
    food_license_url        TEXT,           -- For food merchants (FSSAI)
...
CREATE TABLE merchant_bank_details (
    ...
    upi_id              VARCHAR(100),
    razorpay_contact_id TEXT,              -- Razorpay linked account
...
CREATE TABLE categories (
    ...
    image_url       TEXT,
    icon_url        TEXT,
    theme_color     VARCHAR(20),                    -- hex color per brand guidelines
...
CREATE TABLE products (
    ...
    barcode             VARCHAR(100),
    brand               VARCHAR(100),
    unit                VARCHAR(50),                    -- kg, litre, piece, dozen
    -- Pricing
...
CREATE TABLE product_variants (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    product_id      UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    variant_name    VARCHAR(100) NOT NULL,              -- "500g" | "Red" | "XL"
    variant_type    VARCHAR(50) NOT NULL,               -- weight | color | size
    mrp             DECIMAL(12, 2) NOT NULL,
    retail_price    DECIMAL(12, 2) NOT NULL,
    wholesale_price DECIMAL(12, 2),
    stock_quantity  INT NOT NULL DEFAULT 0,
...
CREATE TABLE service_slots (
    ...
    is_blocked      BOOLEAN NOT NULL DEFAULT FALSE,     -- manual staff block
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE bookings (
    id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    booking_number      VARCHAR(30) UNIQUE NOT NULL,    -- MLB-BK-2026-XXXXXX
    ...
    address_id          UUID REFERENCES user_addresses(id),    -- for home visits
    notes               TEXT,
...
CREATE TABLE carts (
    ...
    merchant_id     UUID REFERENCES merchants(id),       -- Single-merchant cart restriction
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- ============================================================
-- Flow: payment_pending → payment_processed → merchant_approved/rejected
--       → (admin_overridden) → accepted → packed → out_for_delivery → delivered
-- ============================================================

CREATE TABLE orders (
    id                      UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    order_number            VARCHAR(30) UNIQUE NOT NULL,    -- MLB-ORD-2026-XXXXXX
    user_id                 UUID NOT NULL REFERENCES users(id),
    merchant_id             UUID NOT NULL REFERENCES merchants(id),
    ...
    -- Double-Approval Logic (CRITICAL)
    order_status            order_status NOT NULL DEFAULT 'payment_pending',
    payment_processed_at    TIMESTAMPTZ,                    -- Step 1: Payment captured
    merchant_action_at      TIMESTAMPTZ,                    -- Step 2: Merchant approved/rejected
    merchant_action_by      UUID,                          -- merchant user id
    merchant_rejection_reason TEXT,
    admin_override_at       TIMESTAMPTZ,                    -- Admin override
    admin_override_by       UUID,                          -- admin id
    admin_override_note     TEXT,

    -- Area-based availability check
    area_id                 UUID REFERENCES areas(id),      -- customer delivery area
...
CREATE TABLE return_requests (
    ...
    return_items        JSONB NOT NULL,                     -- [{order_item_id, qty, reason}]
    proof_images        TEXT[],
...
CREATE TABLE wallets (
    ...
    owner_type      VARCHAR(20) NOT NULL,       -- customer | merchant
    balance         DECIMAL(12, 2) NOT NULL DEFAULT 0.00,
    locked_balance  DECIMAL(12, 2) NOT NULL DEFAULT 0.00,  -- pending settlements
...
CREATE TABLE wallet_transactions (
    ...
    reference_type      VARCHAR(50),                -- order | refund | referral | withdrawal
    reference_id        UUID,
    description         TEXT,
    initiated_by        UUID,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TABLE delivery_partners (
    ...
    password_hash       TEXT NOT NULL,
    profile_image_url   TEXT,
    vehicle_type        VARCHAR(50),                    -- bike | cycle | van
    vehicle_number      VARCHAR(20),
    aadhaar_number      VARCHAR(20),
    dl_number           VARCHAR(30),                    -- Driving License
    dl_url              TEXT,
    aadhaar_front_url   TEXT,
    aadhaar_back_url    TEXT,
    area_id             UUID REFERENCES areas(id),
...
CREATE INDEX idx_dp_geom    ON delivery_partners USING GIST(current_geom);
...
CREATE TABLE delivery_assignments (
    ...
    failure_reason      TEXT,
    route_taken         JSONB,                              -- GPS breadcrumb trail
...
CREATE TABLE admin_audit_logs (
    ...
    action          VARCHAR(200) NOT NULL,               -- e.g. "approved_merchant"
...
CREATE TABLE admin_device_logs (
    ...
    device_info     JSONB,
    ip_address      INET,
    event           VARCHAR(50) NOT NULL,               -- login | logout | failed_attempt
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE coupons (
    ...
    max_discount_amount DECIMAL(10, 2),                 -- cap for percentage coupons
    min_order_value     DECIMAL(10, 2) DEFAULT 0.00,
    merchant_id         UUID REFERENCES merchants(id),  -- NULL = platform-wide
    category_id         UUID REFERENCES categories(id),
    applicable_for      VARCHAR(20) DEFAULT 'all',      -- all | new_user | referral
    max_uses            INT,
...
CREATE TABLE banners (
    ...
    link_url        TEXT,
    link_type       VARCHAR(50),                        -- category | merchant | product | external
    link_target_id  UUID,
    position        VARCHAR(50) NOT NULL DEFAULT 'hero', -- hero | mid | sidebar | popup
    area_id         UUID REFERENCES areas(id),          -- location-targeted banners
    sort_order      SMALLINT DEFAULT 0,
...
CREATE TABLE complaint_tickets (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    ticket_number   VARCHAR(30) UNIQUE NOT NULL,        -- MLB-TKT-2026-XXXXXX
    user_id         UUID NOT NULL REFERENCES users(id),
    order_id        UUID REFERENCES orders(id),
    booking_id      UUID REFERENCES bookings(id),
    merchant_id     UUID REFERENCES merchants(id),
...
CREATE TABLE sponsored_ads (
    ...
    merchant_id     UUID NOT NULL REFERENCES merchants(id) ON DELETE CASCADE,
    product_id      UUID REFERENCES products(id),
    ad_type         VARCHAR(50) NOT NULL,               -- product_boost | store_banner | category_top
    area_id         UUID REFERENCES areas(id),
...
CREATE TABLE merchant_settlements (
    id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    merchant_id         UUID NOT NULL REFERENCES merchants(id),
    settlement_period   VARCHAR(20),                    -- e.g. "2026-W20"
...
-- ============================================================
-- SECTION 22: AREA-BASED AVAILABILITY CHECK VIEW
-- ============================================================
...
SELECT
    ...
    CASE
        WHEN ST_Distance(m.geom::geography, a.geom::geography) / 1000
             <= m.delivery_radius_km THEN TRUE
        ELSE FALSE
...
-- ============================================================
-- SECTION 23: UPDATED_AT AUTO-TRIGGER FUNCTION
-- ============================================================
...
-- ============================================================
-- SECTION 24: SEED DATA — CATEGORIES
-- ============================================================
...
-- Update PostGIS geom column from lat/lng
UPDATE areas SET geom = ST_MakePoint(longitude, latitude)::geography WHERE geom IS NULL;
...
-- END OF SCHEMA
-- ============================================================
-- Tables: 40+ | Indexes: 50+ | Triggers: 20+ | Views: 1
-- Covers: Auth, Location, Merchants, Products, Services,
--         Orders (Double-Approval), Delivery, Payments,
--         Wallet, Admin, Marketing, CMS, Settlements
-- ============================================================
